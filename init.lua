...
vim.o.tags=os.getenv("VIM_TAG_FILE")
...
end, { noremap=true, desc = '[si] Toggle sign column' })
...
vim.keymap.set('n', '<leader>edate', function()
  -- Get the current date and time
  local date = os.date("%Y-%m-%d %H:%M:%S")
  -- Insert the date and time at the cursor position
  vim.api.nvim_buf_set_lines(0, -1, -1, false, {date})
end, { noremap=true, desc = '[edate] Enter current date' })

vim.keymap.set('n', '<leader>w', ':update!<CR>',{ noremap=true, desc='Write only if buffer modified.' })
...
vim.keymap.set({'n', 'v', 'x', 'o'}, 'K', 'k', { noremap=true, desc='uppercase K to k' })
